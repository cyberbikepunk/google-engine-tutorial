...
def guestbook_key(guestbook_name=DEFAULT_GUESTBOOK_NAME):
    """ Constructs a Datastore key for a Guestbook entity. We use guestbook_name as the key. """
...
class MainPage(RequestHandler):
    def get(self):
        ...
        for greeting in greetings:
            ...
            self.response.write('<blockquote>%s</blockquote>' % escape(greeting.content))
        ...
        sign_query_params = urlencode({'guestbook_name': guestbook_name})
        self.response.write(MAIN_PAGE_FOOTER_TEMPLATE % (sign_query_params, escape(guestbook_name), url, url_linktext))
